  ...
  <div class="modal-dialog modal-sm">
    <div class="modal-content">
      ...
      <div class="modal-body"> 
      	 Are you sure want to cancel this "<strong> <?php echo $vacantsea["vacantsea"]; ?> </strong>" Applicant  ?         
      </div>
...
<script>
	...
	function delete_competency_tr()
	{
		...
		$.ajax({
			...
			type:"post",
			url:"<?php echo base_url("seaman/resume_process/cancel_apply_vacantsea") ?>",
			data:"x=1&id_update=<?php echo $applicant['id_aplicant'] ?>",
			success: function(data){
					//alert('<?php echo $applicant[id_aplicant] ?>');
				$(".modal-header").css({'font-color':'black'});
				$(".modal-footer").hide();
				$(".modal-body").html("You have cancel <?php echo $vacantsea['vacantsea'] ?> Vacantsea ");
				setTimeout(function() { 
