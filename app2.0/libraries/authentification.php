...
class authentification {
	...
	function underconst_access()
	{
		...
		if($underconstruction["set"] == TRUE)
		{
			...
			{
				header("location:".base_url("under_const"));
			}
		}
	}
	...
	function reserve()
	{
		...
		if($username_url == $username_sess) return true;
		else if($username_url == "" && $username_sess != "") return true;
		else
		{
			return false;	
		}
	...
	function cek_tampil($usernamenya){
		$CI =& get_instance();

		// if($id == ""){
		// 	$id = $CI->session->userdata('id_perusahaan_agent');
		// }

		$sql = "SELECT * FROM perusahaan WHERE username = '$usernamenya'";
		...
		if($f != null){
			/* Jika data ada */
			$username_session = $CI->session->userdata('username_company');

			if($usernamenya != $username_session)
			{
				/* Jika username di url dan username yang sedang login tidak sama */
				if($f['tampil'] == 0) show_404();
			}
		}
		else show_404();
