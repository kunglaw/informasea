...
function image_small_fr($file_name = "",$destination = "resume", $source = "resume")
{
	...
	$config['image_library'] = 'gd2';
	$config['source_image'] = pathup("photo/$username/$source/$explodesi[0]"."_thumb.".$explodesi[1]);
...
//  $username 	= $CI->session->userdata("username");
//  
//  $x_axis 	  = $CI->input->post("x1");
//  $y_axis 	  = $CI->input->post("y1");
//  $width	   = $CI->input->post('width'); // lebar gambar yg di form
//  $height 	  = $CI->input->post("height"); // panjang gambar yg di form
//  $wcrop 	   = $CI->input->post("wcrop"); // width crop
//  $hcrop	   = $CI->input->post("hcrop"); // height crop
//
//  $config['image_library'] 	= 'gd2';
//  $config['source_image'] 	 = pathup("photo/$username/$source/$file_name");
//  
...
function image_small_cropp($file_name = "",$destination = "profile_pic")
{
	$CI =& get_instance();
	$CI->load->library("image_lib");
	$CI->load->helper('file_url');
	...
	$explodesi = explode(".",$file_name);
	$exxplode = explode("_",$explodesi[0]);
	$config['new_image']	= pathup("photo/$username/$destination/$exxplode[0]"."_small.".$explodesi[1]);
	//$config['create_thumb'] = TRUE;
	//$config['thumb_marker'] = "_small";
	//$config['quality'] = '85%';
	$config['maintain_ratio'] = FALSE;
