...
if(!function_exists("track_print_resume"))
{
	function track_print_resume($arr)
	{
	  $CI     =& get_instance();
	  $CI->load->model("tracker_model","tracker");
	  ...
	  $action_arr = array("username"=>$username_login,
	  "action"=>"print",
	  "category"=>"seatizen",
	  "page"=>"resume_print",
	  "type"=>$type);
	  
	  $action = json_encode($action_arr);

	  if(!empty($username_login))
	  {

		  $seatizen_id = !empty($id_seatizen) ? $id_seatizen : 0;
          $CI->tracker->save_seatizen_action($id_seatizen, $action);
