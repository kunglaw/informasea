...
if(!function_exists('ajax_url'))
{
	...
	{
		$CI =& get_instance();
		$ajax_url = $CI->config->item("ajax_url");
		
		return $ajax_url."/".$path;
...
if(!function_exists('alpha_url'))
{
	function alpha_url($path)
	{
		$CI =& get_instance();
		$CI->load->config("asset");
		$alpha_url = $CI->config->item("alpha_url");
...
if(!function_exists('agent_url')){
	...
	function agent_url($path){
		$CI =& get_instance();
		$agent_url = $CI->config->item("agent_url");
