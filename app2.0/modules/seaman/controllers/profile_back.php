<?php if(!defined('BASEPATH')) exit ('No Direct Script Access Allowed '); 
	...
	class Profile extends MX_Controller
	{
		...
		public function index()
		{
			...
			// meta 
				...
				$rankd = "";
				if(!empty($rank["rank"])) $rankd = "$rank[rank] for $years years";
				
				$desc = "Hello seafarer, iam $name from $school $pelaut[kebangsaan]. iam $rankd. 
	...
	//				$arr['header']   		  = "seatizen/header";
		...
		function recommendation()
		{
			...
			$data['pelaut']  		 = $this->seatizen_model->detail_seatizen($username_url);
			...
			foreach($exp2 as $row)
			{
				$data["experience"][] = $this->experience_model->get_experience_detail($exp2);
				
			}
			
			//echo $data["pelaut"]["pelaut_id"]; exit;
			
			//$data["recommendation"] = $this->recommendation_model->rec_by_pelaut($data["pelaut"]["pelaut_id"]);
			
			//print_r($data["recommendation"]); 
