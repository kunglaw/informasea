...
<?php 
  ...
  foreach($seatizen_all as $row):
	...
	if($crl["result"] == TRUE && $ii <= 2){
	  
	 
	  // pembatasan cuma 2
?>

   <div class="col-md-<?php echo $col; ?> seatizen-item-container-complete">
...
<br />
  ...
  <!--          <li style="z-index:1000;"><a onclick="batal_terima(<?php echo $id; ?>)"> Cancel Request </a> </li> -->  
            ...
            <button class="btn btn-filled pull-left btn-custom-font" onclick="addFriend(<?php echo $id; ?>)" id="btnaddfriend_<?php echo $row['pelaut_id'];?>">
		     ...
		     <div class="btn-group dropup">
             <button class="btn btn-success pull-left btn-custom-font dropdown-toggle" data-toggle="dropdown" id="btnwaiting_<?php echo $id; ?>">  
			 <?=$this->lang->line("waiting")?> .. </button>
         ...
         <div class="btn-group dropup">
                ...
                id="btndelfriend_<?php echo $row['pelaut_id']; ?>">   <?=$this->lang->line("friend")?> </button>
                  <ul class="dropdown-menu" role="menu">
                     ...
                     <li><a onclick="javascript:deletefriend(<?php echo $row['pelaut_id']; ?>)"> <i class="fa fa-minus"> 
                     	</i>  <?=$this->lang->line("unfriend")?> </a> 
                     </li>
                  </ul>
        </div>
          ...
          <?php  }//echo "tes('$id','$visFriend', '$visRequest','$visDelFriend');" ?>
        	<?php } else  if($id_user == "" AND empty($company))  { ?>
                ...
                <button class="btn btn-filled pull-left btn-custom-font" onclick="login()"> <i class="fa fa-plus"> </i> 
            ...
            <a class="btn btn-filled pull-left btn-custom-font" href="<?php echo base_url("profile/".$row['username']."/resume") ?>"> <?=$this->lang->line("view")?> Resume </a>
          <?php  }	
            ?>
            ...
            <!--<div class="pull-left" style="margin-left:10px;">|</div>-->
            <div class="pull-left" style="margin-left:10px;">
            	<span title="Experience in Rank <?=$rank['rank']?>" style="color:#999; font-size: 9pt">
                &nbsp;<i class="glyphicon glyphicon-list-alt"></i>
            	<?php
					echo $selisihnya = count_experience($row["pelaut_id"]);
            	?>
                </span>
            </div>
...
<script>
tes('<?php echo $id; ?>','<?php echo $visFriend; ?>','<?php echo $visRequest; ?>','<?php echo $visDelFriend; ?>','<?php echo $visConfirm; ?>',".seatizen-item-container-complete");
</script>
<?php
	
	$ii++;
	
  }// end check resume lengkap
