<?php
	...
	class company_model extends CI_Model{
		...
		function company_index($count=false,$offset=0){
			$str = "SELECT * FROM perusahaan WHERE activation_code = 'ACTIVE' AND tampil = 1 order by create_date DESC";
		...
		public function count_company(){
			...
			$q = $this->db->query("SELECT * FROM perusahaan WHERE activation_code = 'ACTIVE' AND tampil = 1 ");
			$f = $q->num_rows();
			return $f;
		}
		...
		{
			$str = "select * from perusahaan where activation_code = 'ACTIVE' AND tampil = 1 order by create_date DESC LIMIT 5";
			$q = $this->db->query($str);
			$f = $q->result_array();
		...
		public function list_company_lim()
		{
			...
			$str = "select * from perusahaan where activation_code = 'ACTIVE' AND tampil = 1 order by id_perusahaan desc limit 5";
			$q = $this->db->query($str);
			$f = $q->result_array();			
			return $f;
		}
		...
		{	
			...
			$str = "select * from perusahaan where activation_code = 'ACTIVE' and id_perusahaan < '$id_perusahaan'  AND tampil = 1 order by id_perusahaan desc limit 5"; //harusnya ada order by createdate
			$q = $this->db->query($str);
