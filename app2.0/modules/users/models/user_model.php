...
class User_model extends CI_Model{
	...
	public function check_activation_company()
	{
		...
		/* echo $str = "select * from perusahaan where username = '$username_url' and email = '$email_url' and password = '$password' and activation_code = '$activate_url' "; */
		
		$str = "select * from perusahaan where username = '$username_url' and email = '$email_url' and password = '$password'";
...
//        if(empty($password) && empty($email)) $str.=" username = '$username'";
	...
	public function update_activation_company2017($email)
	{
		
		$str2 = "SELECT * FROM perusahaan WHERE email = '$email' ";
		$q2 = $this->db->query($str2);
		$f2 = $q2->row_array();
		
		$str = "update perusahaan set valid_email = 'VALID' where id_perusahaan = '$f2[id_perusahaan]' ";
		$q = $this->db->query($str);
	...
	public function check_auser($username) {
		...
		AND activation != 'BLOCKED' "; // harus user yg active
		//echo $sql;
		$q = $this->db->query($sql);
		$r=$q->row_array();
		return $r; 
	...
	public function check_nauser($username)
	{
		...
		//echo $username;
		$sql = "SELECT * FROM pelaut_ms WHERE ( username = '$username' or email = '$username' ) and password = '$password' 
		and activation != 'ACTIVE' AND activation != 'BLOCKED'";
		//echo $sql;
		$q = $this->db->query($sql);
		$r=$q->row_array();
		return $r; 	
	...
	function check_blocked_user($username)
	{
		$sql = "SELECT * FROM pelaut_ms WHERE (username = '$username' OR email = '$username') AND activation = 'BLOCKED' ";
		$q   = $this->db->query($sql); 
		$f   = $q->row_array();
