...
class Proficiency_model extends CI_Model{
	...
	function get_proficiency_tr($pelaut_id='',$id_sertifikat='')

	{

		if(empty($pelaut_id))     $pelaut_id 	 = $this->session->userdata('id_user');

		if(empty($id_sertifikat)) $id_sertifikat = $this->input->post("id_update"); // id_update dari javasecript
...

	{
		...
		// $this->form_validation->set_rules("expired_date","Expired date","required"); //sementara di hapus 
