...
class Experience_model extends CI_Model{
	...
	function total_experience_row($experience_id)
	{
		$sql 	= "SELECT periode_from, periode_to , SUM(datediff(periode_to, periode_from)) as total_experience 
		FROM experiences WHERE experience_id = '$experience_id' ";
		...
		if ($total_hari >= 365) {
			...
			return $tahun. " years experiences"; //.$bulan." months, ".$hari." days";                           

		}elseif ($total_hari >= 30) {
			$bulan      = floor($total_hari/30);
			$hari       = $total_hari%30;
			
			return $bulan. " months experiences"; //.$hari." days";

		}elseif ($total_hari >= 1) {
			
			return $total_hari ." days experiences";
	...
	function total_experience($id_pelaut = 5)
	{
	...
	function experience($id_pelaut = 5)
	{
		 $e  = $this->total_experience($id_pelaut);
		
		 $total_hari = $e["total_experience"];
	...
	function get_experience_detail($experience_id = "") // by experience_id
	{
...

	{
		...
		if($this->form_validation->run() == TRUE && $periode_to >= $periode_from)
...

		{
			...
			{
				$err_add = "<p> Sign off must be higher than Sign on </p>";
			}

			echo "<div class='alert alert-danger'>".$err_add.validation_errors()."</div>";

			echo '<script>$("#add-experience-btn").button("reset");</script>';
